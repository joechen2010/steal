...
if($action=='mod'){
	...
	$Fid_db = include(dirname(__FILE__)."/../data/all_fid.php");
	...
	if($fiddb){
		...
		$fiddb_string && $SQL.=" AND A.fid IN ($fiddb_string) ";
	}
	$i=$sql_DB='';
	$sql_DB[]="(SELECT A.*,B.* FROM {$_pre}content A LEFT JOIN {$_pre}content_1 B ON A.id=B.id $SQL)";
	...
	foreach( $Fid_db[tableid] AS $key=>$value){
		...
		$i++;
		$sql_DB[$value]="(SELECT A.* FROM {$_pre}content$value  A LEFT JOIN {$_pre}content_1 B ON A.id=B.id $SQL)";
	}
	if($i&&$i==count($fiddb)){
		unset($sql_DB[0]);
	}
	...
	if(count($sql_DB)>1){
		...
		if($NUM>10000){
			showerr("你只能选择同一张分表的栏目,不同张分表的栏目不允许同时调用,否则极其影响速度");
		}
	}
