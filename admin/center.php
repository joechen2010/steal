...
elseif($job=="map")
{
	...
	@extract($db->get_one("SELECT COUNT(*) AS companyNM FROM {$pre}hy_company"));
	//@extract($db->get_one("SELECT COUNT(*) AS memberNUM FROM {$pre}memberdata"));
	//@extract($db->get_one("SELECT COUNT(*) AS showroomNUM FROM {$pre}showroom_room"));
	@extract($db->get_one("SELECT COUNT(*) AS sellNUM FROM {$pre}sell_content"));
	@extract($db->get_one("SELECT COUNT(*) AS buyNUM FROM {$pre}buy_content"));
	//@extract($db->get_one("SELECT COUNT(*) AS brandNUM FROM {$pre}brand_content"));
	//@extract($db->get_one("SELECT COUNT(*) AS zhNUM FROM {$pre}showroom_content"));
	$memberNUM = $userDB->total_num();
...
elseif($job=='GetOfficialNotice'){
	...
	}if( !@copy("http://$OfficialUrl1$OfficialUrl2/OfficialNotice.php?E=$qibosoft_Edition&upgrade_Record=$webdb[upgrade_Record]&U=$webdb[www_url]&T=B2B",ROOT_PATH."cache/OfficialNotice.txt") )
	{
		if(!$OfficialNotice=file_get_contents("http://$OfficialUrl1$OfficialUrl2/OfficialNotice.php?E=$qibosoft_Edition&upgrade_Record=$webdb[upgrade_Record]&U=$webdb[www_url]&T=B2B"))
		{
			$OfficialNotice=sockOpenUrl("http://$OfficialUrl1$OfficialUrl2/OfficialNotice.php?E=$qibosoft_Edition&upgrade_Record=$webdb[upgrade_Record]&U=$webdb[www_url]&T=B2B");
		}
